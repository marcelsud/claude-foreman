...
from claude_foreman.doctor import run_doctor
...
class DoctorTests(unittest.TestCase):
    def test_codex_check_uses_same_subscription_provider_as_worker(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (
                patch("claude_foreman.doctor.importlib.util.find_spec", return_value=None),
                patch("claude_foreman.doctor.shutil.which", side_effect=which),
                patch("claude_foreman.doctor.subprocess.run", return_value=completed) as run,
            ):
