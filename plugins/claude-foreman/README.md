...
The installed bridge uses `~/.local/share/claude-foreman/runtime`, injects its own cached `src/` into `PYTHONPATH`, and keeps SQLite state under `~/.local/share/claude-foreman`. A source checkout falls back to its ignored `.foreman-data/` only when the manager sandbox cannot write the home directory.
