...
if "FOREMAN_DATA_DIR" not in os.environ:
    preferred_data = Path.home() / ".local" / "share" / "claude-foreman"
    try:
...
shared_venv_python = (
    Path.home() / ".local" / "share" / "claude-foreman" / "runtime" / "bin" / "python"
)
...
from claude_foreman.mcp_server import main  # noqa: E402
