...
from claude_foreman.runner import ClaudeWorker
from claude_foreman.worktrees import Worktree
...
class RunnerSafetyTests(unittest.IsolatedAsyncioTestCase):
    async def test_worker_fails_closed_before_creating_worktree_without_sandbox(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch("claude_foreman.runner.missing_sandbox_dependencies", return_value=["socat"]):
    ...
    async def test_daemon_shutdown_requeues_in_progress_task(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (
                patch("claude_foreman.runner.missing_sandbox_dependencies", return_value=[]),
